// Generic LLM provider. Calls the `generate` Cloud Function for a given provider
// (claude/openai/gemini), then runs every candidate through the SAME client-side
// gate as the mock provider (banned-opener rejection + similarity-vs-recent),
// regenerating up to the per-business limit. The uniqueness/quality engine stays
// the single source of truth; only the model behind the function differs.
...
export type LlmName = Exclude<ProviderName, 'mock'>;
...
export class LlmContentProvider implements ContentProvider {
  constructor(
    readonly name: LlmName,
    private businessId: string,
    private transport: GenerateTransport = callGenerate,
  ) {}

  private record(type: GenerationType, text: string, recent: string[], brand: BrandSettings, regen: number): GeneratedRecord {
  ...
  private async run<T>(
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      const res = await this.transport({ provider: this.name, businessId: this.businessId, kind, payload, brand: toBrandLite(brand), avoid });
  ...
  async generateContent(req: GenerationRequest, brand: BrandSettings, recent: RecentByType): Promise<ProviderOutput<GenerationResult>> {
    ...
    const records = [
      ...
      this.record('caption', result.caption!.text, recent.caption ?? [], brand, regen),
      this.record('cta', result.cta!.text, recent.cta ?? [], brand, regen),
    ];
    result.quality = avgQuality(records.map((r) => scoreOutput(r.text, [], brand)));
    return { result, records, cost: costFromTokens(this.name, tokens, timeMs, regen) };
  }
  ...
  async generateScript(req: ScriptRequest, brand: BrandSettings, recent: RecentByType): Promise<ProviderOutput<ScriptResult>> {
    ...
    const records = [
      this.record('hook', result.hook, recent.hook ?? [], brand, regen),
      this.record('script', result.script, recent.script ?? [], brand, regen),
    ];
    return { result, records, cost: costFromTokens(this.name, tokens, timeMs, regen) };
  }
  ...
  async generateReviewResponse(req: ReviewRequest, brand: BrandSettings, recent: RecentByType): Promise<ProviderOutput<ReviewResult>> {
    ...
    const records = [
      this.record('review', result.short, recent.review ?? [], brand, regen),
      this.record('review', result.professional, recent.review ?? [], brand, regen),
      this.record('review', result.seoFriendly, recent.review ?? [], brand, regen),
    ];
    return { result, records, cost: costFromTokens(this.name, tokens, timeMs, regen) };
  }
  ...
  async generateSocialReply(req: SocialRequest, brand: BrandSettings, recent: RecentByType): Promise<ProviderOutput<SocialResult>> {
    ...
    const records = result.replies.map((r) => this.record('reply', r, recent.reply ?? [], brand, regen));
    return { result, records, cost: costFromTokens(this.name, tokens, timeMs, regen) };
  }
  ...
  async repurposeContent(req: RepurposeRequest, brand: BrandSettings, recent: RecentByType): Promise<ProviderOutput<RepurposeResult>> {
    ...
    const records = [
      ...result.hooks.map((h) => this.record('hook', h, recent.hook ?? [], brand, regen)),
      ...result.captions.map((c) => this.record('caption', c, recent.caption ?? [], brand, regen)),
    ];
    return { result, records, cost: costFromTokens(this.name, tokens, timeMs, regen) };
  }
}

export function makeLlmProvider(name: LlmName, businessId: string, transport?: GenerateTransport): LlmContentProvider {
  return new LlmContentProvider(name, businessId, transport);
}
